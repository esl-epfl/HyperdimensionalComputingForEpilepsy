...
class FeaturesParams:
    ...
    indivFeatNames_MeanAmpl=['mean_ampl']
    indivFeatNames_LL=['line_length']
...
class ZeroCrossFeatureParams:
    ...
    winLen=4 # in sec
    winStep=0.5 # in sec
    minValueSignal = np.zeros((len(DatasetPreprocessParams.channelNamesToKeep)))
    meanValueSignal = np.zeros((len(DatasetPreprocessParams.channelNamesToKeep)))  # variable to keep track in real time about min and max value
