...
#
#     #probability
#     # probabilityLab=distFromCorrClass
#     # predLabels[s], distances[s, :]
...
#
#     # dist_fromVotedOne=distances[:,predLabels.astype(int)]
#     # dist_oppositeVotedOne = np.mean(distances[not predLabels, :],0)
#     # probabilityLab1 = ((1 - distFromCorrClass) - (1 - distFromWrongClass)) / (1 - distFromCorrClass)
#     probabilityLab_confidence=( (1-dist_fromVotedOne)-(1-dist_oppositeVotedOne) ) /(1-dist_fromVotedOne)
#
#     probabilityLab =dist_oppositeVotedOne/ (dist_oppositeVotedOne + dist_fromVotedOne)
#     return(acc, accPerClass, distWhenWrongPredict_mean, distFromCorr_AllClass,distFromCorr_PerClass,  distFromWrong_AllClass, distFromWrong_PerClass, predLabels, probabilityLab)
#
...
#
#
# def testModelVecOnData_MeasureDistances(data, trueLabels, model, ModelVectors, HDParams):
#     (unique_labels, counts) = np.unique(trueLabels, return_counts=True)
#     (numLabels, totalD)=ModelVectors.shape
#     (numWin, x) = data.shape
#     predLabels = np.zeros((numWin, HDParams.numFeat+1))
#     distances=np.zeros((numLabels, HDParams.numFeat+1))
#     distFromS=np.zeros((numWin, HDParams.numFeat+1))
#     distFromNS = np.zeros((numWin, HDParams.numFeat+1))
#
#     for s in range(numWin):
#         (temp, timeHDVec) = model.learn_HD_proj(data[s, :], HDParams)
#         tempVec=temp.cpu().numpy()
...
#
#
def testAndSavePredictionsForHD_PerFeat(folderIn, fileName, model, ModelVectorsNorm, HDParams, data_train_ToTrain, label_train,data_test_ToTrain, label_test, HDtype):
    (predLabels_train, distFromS_train, distFromNS_train) = testModelVecOnData_MeasureDistances(data_train_ToTrain, label_train, model,ModelVectorsNorm, HDParams)
    (predLabels_test, distFromS_test, distFromNS_test) = testModelVecOnData_MeasureDistances(data_test_ToTrain, label_test, model, ModelVectorsNorm, HDParams)
    # predictions
    dataToSave = np.hstack((predLabels_train, np.reshape(label_train, (len(label_train), -1))))
    outputName = folderIn + '/' + fileName + '_'+HDtype+'_PerFeat_TrainPredictions.csv'
    saveDataToFile(dataToSave, outputName, 'gzip')
    dataToSave = np.hstack((predLabels_test, np.reshape(label_test, (len(label_test), -1))))
    outputName = folderIn + '/' + fileName + '_'+HDtype+'_PerFeat_TestPredictions.csv'
    saveDataToFile(dataToSave, outputName, 'gzip')
    # distances from seiz and non seiz class
    outputName = folderIn + '/' + fileName + '_'+HDtype+'_PerFeat_DistancesFromSeiz_Train.csv'
    saveDataToFile(distFromS_train, outputName, 'gzip')
    outputName = folderIn + '/' + fileName +  '_'+HDtype+'_PerFeat_DistancesFromNonSeiz_Train.csv'
    saveDataToFile(distFromNS_train, outputName, 'gzip')
    outputName = folderIn + '/' + fileName +  '_'+HDtype+'_PerFeat_DistancesFromSeiz_Test.csv'
    saveDataToFile(distFromS_test, outputName, 'gzip')
    outputName = folderIn + '/' + fileName + '_'+HDtype+'_PerFeat_DistancesFromNonSeiz_Test.csv'
...
#
#
# def calcualtePerformanceMeasures_allSmoothng(predictions, trueLabels,  toleranceFP_bef, toleranceFP_aft, numLabelsPerHour, seizureStableLenToTestIndx,  seizureStablePercToTest,distanceBetweenSeizuresIndx):
#
#     performancesAll=performance_all9(predictions, trueLabels, toleranceFP_bef, toleranceFP_aft, numLabelsPerHour)
#     # smoothen labels
#     (yPred_SmoothOurStep2, yPred_SmoothOurStep1) = smoothenLabels(predictions, seizureStableLenToTestIndx, seizureStablePercToTest, distanceBetweenSeizuresIndx)
#
#     performancesAll_step1= performance_all9(yPred_SmoothOurStep1, trueLabels, toleranceFP_bef, toleranceFP_aft,numLabelsPerHour)
#     performancesAll_step2 = performance_all9(yPred_SmoothOurStep2, trueLabels, toleranceFP_bef, toleranceFP_aft, numLabelsPerHour)
#     return(performancesAll, performancesAll_step1, performancesAll_step2,yPred_SmoothOurStep1,yPred_SmoothOurStep2 )
#
#
# def func_analysePerFeature(folderIn, folderOut, GeneralParams, PostprocessingParams, FeaturesParams, HDParams, HDtype):
#     # various postpocessing parameters
#     seizureStableLenToTestIndx = int(PostprocessingParams.seizureStableLenToTest / FeaturesParams.winStep)
#     seizureStablePercToTest = PostprocessingParams.seizureStablePercToTest
#     distanceBetweenSeizuresIndx = int(PostprocessingParams.distanceBetween2Seizures / FeaturesParams.winStep)
...
#     confidencesPerFeat_Train_AllSubj= np.zeros((len(GeneralParams.patients), HDParams.numFeat ))
#     confidencesPerFeat_Test_AllSubj = np.zeros((len(GeneralParams.patients), HDParams.numFeat ))
#     performancesAll_Train_AllSubj = np.zeros((HDParams.numFeat + 1, 36, len(GeneralParams.patients)))
#     performancesAll_Test_AllSubj = np.zeros((HDParams.numFeat + 1, 36, len(GeneralParams.patients)))
#
...
#         confidencesPerFeat_Train_ThisSubj = np.zeros((numFiles, HDParams.numFeat ))
#         confidencesPerFeat_Test_ThisSubj = np.zeros((numFiles, HDParams.numFeat ))
#         performancesAll_Train_ThisSubj = np.zeros((HDParams.numFeat + 1, 36, numFiles))
#         performancesAll_Test_ThisSubj = np.zeros((HDParams.numFeat + 1, 36, numFiles))
#
...
#             #predictions
#             data0 = readDataFromFile(folderOut+ fNameBase+'_PerFeat_TrainPredictions.csv')
#             predLabels_train=data0[:,0:-1]
#             label_train=data0[:,-1]
#             data0 = readDataFromFile(folderOut+fNameBase+'_PerFeat_TestPredictions.csv')
#             predLabels_test=data0[:,0:-1]
#             label_test=data0[:,-1]
#             #distances
#             distFromS_train = readDataFromFile(folderOut+ fNameBase+'_PerFeat_DistancesFromSeiz_Train.csv.gz')
#             distFromNS_train = readDataFromFile(folderOut+ fNameBase+'_PerFeat_DistancesFromNonSeiz_Train.csv.gz')
#             distFromS_test = readDataFromFile(folderOut + fNameBase + '_PerFeat_DistancesFromSeiz_Test.csv.gz')
#             distFromNS_test = readDataFromFile(folderOut + fNameBase + '_PerFeat_DistancesFromNonSeiz_Test.csv.gz')
#
#             # COMPARE FEATURES
...
#             outputName = folderOut  + fNameBase + '_PerFeat_TrainConfidence.csv'
#             saveDataToFile(confidencesPerFeat_Train, outputName, 'gzip')
#             outputName =folderOut  + fNameBase + '_PerFeat_TestConfidence.csv'
...
#             # saving
#             outputName = folderOut + '/' + fNameBase + '_PerFeat_TrainPerformance.csv'
#             saveDataToFile(performancesAll_Train, outputName, 'gzip')
#             outputName = folderOut + '/' + fNameBase + '_PerFeat_TestPerformance.csv'
...
#             # ax1.plot(xValues, performancesAll_Train[0:-1, 2], 'k--')
#             # ax1.plot(xValues, performancesAll_Train[0:-1, 5], 'b--')
#             # ax1.plot(xValues, performancesAll_Train[0:-1, 7], 'm--')
...
#             # # ax1.plot(xValues, performancesAll_step1_Train[0:-1, 2], 'k')
#             # # ax1.plot(xValues, performancesAll_step1_Train[0:-1, 5], 'b')
#             # # ax1.plot(xValues, performancesAll_step1_Train[0:-1, 7], 'm')
...
#             # ax1.plot(xValues, performancesAll_Test[0:-1, 2], 'k--')
#             # ax1.plot(xValues, performancesAll_Test[0:-1, 5], 'b--')
#             # ax1.plot(xValues, performancesAll_Test[0:-1, 7], 'm--')
#             # ax1.legend(['F1E', 'F1D', 'F1both'])
...
#             # # ax1.plot(xValues, performancesAll_step1_Test[0:-1, 2], 'k')
#             # # ax1.plot(xValues, performancesAll_step1_Test[0:-1, 5], 'b')
#             # # ax1.plot(xValues, performancesAll_step1_Test[0:-1, 7], 'm')
...
#             # ax1.plot(xValues, distSeparability[0:-1], 'k')
...
#             # ax1.plot(xValues, confidencesPerFeat_Train, 'b')
#             # ax1.plot(xValues, confidencesPerFeat_Test, 'r')
#             # ax1.legend(['Train', 'Test'])
#             # # ax1.plot(xValues, np.ones(len(xValues)) * confidencesPerFeat_Train[-1], 'b')
#             # # ax1.plot(xValues, np.ones(len(xValues)) * confidencesPerFeat_Test[-1], 'r')
#             # ax1.set_xlabel('Features')
#             # ax1.set_title('Vector confidence')
#             # ax1.grid()
#             # # correlation separability adn prformance
#             # ax1 = fig1.add_subplot(gs[1, 0])
#             # ax1.plot(distSeparability[0:-1], performancesAll_Train[0:-1, 7], 'bx')
#             # ax1.plot(distSeparability[0:-1], performancesAll_Test[0:-1, 7], 'rx')
#             # ax1.legend()
...
#             # ax1.legend(['Train', 'Test'])
#             # ax1.grid()
#             # # correlation confidence adn prformance
#             # ax1 = fig1.add_subplot(gs[1, 1])
#             # ax1.plot(confidencesPerFeat_Train, performancesAll_Train[0:-1, 7], 'bx')
#             # ax1.plot(confidencesPerFeat_Test, performancesAll_Test[0:-1, 7], 'rx')
#             # ax1.legend()
...
#             # ax1.plot(distSeparability[0:-1], confidencesPerFeat_Train, 'bx')
#             # ax1.plot(distSeparability[0:-1], confidencesPerFeat_Test, 'rx')
#             # ax1.legend()
#             # ax1.set_xlabel('Separability')
...
#             # ax1.grid()
#             # fig1.show()
#             # fig1.savefig(folderOut + '/' + fNameBase + '_FeatureComparison.png')
#             # plt.close(fig1)
#
...
#     ax1.set_xticklabels(FeaturesParams.featNames, fontsize=8 * 0.8, rotation=30)
#     ax1.set_title('Vector separability')
#     ax1.grid()
#     # vector confidence
#     ax1 = fig1.add_subplot(gs[0, 1])
...
#     ax1.grid()
#     fig1.show()
#     fig1.savefig(folderOut + '/AllSubj'+ '_'+ HDtype+'_FeatureComparison.png')
#     fig1.savefig(folderOut + '/AllSubj'+ '_'+ HDtype+'_FeatureComparison.svg')
#     plt.close(fig1)
#
#
def plotPerFeatureComparison_ForPaper(folderOut, HDParams,FeaturesParams,  HDtype):
    ...
    # ax1.set_xlabel('Features')
    ax1.set_xticks(xValues)
    ax1.set_xticklabels(FeaturesParams.featNames, fontsize=8 * 0.8, rotation=90)
    ax1.set_title('Vector separability')
    ax1.grid()
    # vector confidence
    ax1 = fig1.add_subplot(gs[0, 1])
    ...
    ax1.grid()
    fig1.show()
    fig1.savefig(folderOut + '/AllSubj' + '_' + HDtype + '_FeatureComparison_forPaper.png')
    fig1.savefig(folderOut + '/AllSubj' + '_' + HDtype + '_FeatureComparison_forPaper.svg')
    plt.close(fig1)