...
createFolderIfNotExists( '../'+Dataset) #create folder for dataset if it doesnt exist
...
extractEDFdataToCSV_originalData_gzip(folderInEDF, folderInCSV, DatasetPreprocessParams, GeneralParams.patients )
...
factor=10
folderOutFeatures= '../'+Dataset+'/02_Features_' +datasetFiltering+'_'+str(FeaturesParams.winLen )+'_'+ str(FeaturesParams.winStep)
folderOutRearangedData= '../'+Dataset+'/04_RearangedData_MergedFeatures_' +datasetFiltering+'_'+str(FeaturesParams.winLen )+'_'+ str(FeaturesParams.winStep) +'_' +'-'.join(FeaturesParams.featNames)+'_Fact'+str(factor) +repearabilityVer +'_TheSameFile'#where to save calculated features for each original file
createFolderIfNotExists(folderOutRearangedData)
# concatenateAllFeatures_moreNonseizureForFactor_gzip(folderOutFeatures, folderOutRearangedData,GeneralParams, DatasetPreprocessParams, FeaturesParams,  factor)
concatenateAllFeatures_moreNonseizureForFactorFromTheSameFile_gzip(folderOutFeatures, folderOutRearangedData,GeneralParams, DatasetPreprocessParams, FeaturesParams,  factor)
plotRearangedDataLabelsInTime(folderOutRearangedData,  GeneralParams,PostprocessingParams, FeaturesParams)
