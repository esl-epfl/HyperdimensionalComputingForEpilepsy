...
def performance_duration(y_pred_smoothed, y_true):
    ...
    durationTrueSeizure=y_true.sum().item()

    #total predicted seizure duration
    durationPredictedSeizure=y_pred_smoothed.sum().item()
    ...
    durationTruePredictedSeizure=len(temp[temp==1])
            ...
            for epIndx, ePred in enumerate(predEvents):
                ...
                #     hyp:              <------------------------------------------>
                    ...
                    if (flag_trueEventsFPAround[
                        etIndx - 1] > 0 and fp_bef == 1):  # if there was FP after true seiyure and already counted and now we want to count it again because of before
                        totalFP = totalFP - 1
                ...
                # prdiction but not matched with true seizure
                ...
                elif (flag_predEvents[
                          epIndx] == 2):  # already counted as part of previous seiyure, we dont need to count it again
                    a = 0
                    ...
                    a = 0
                elif (flag_trueEvents[etIndx] == 0 and flag_predEvents[
                    epIndx] == 1):  # pred seiz was matched, true hasnt found yet
                    a = 0
                else:
...
def smoothenLabels(prediction,  seizureStableLenToTestIndx, seizureStablePercToTest, distanceBetweenSeizuresIndx):
    ...
    unfolded_prediction = prediction.unfold(0,seizureStableLenToTestIndx,1).float()
    smoothLabelsStep1 = p(torch.where(unfolded_prediction.mean(1)>=seizureStablePercToTest,1,0))
...
def smoothenLabels_Bayes(prediction,  probability, seizureStableLenToTestIndx,  probThresh, distanceBetweenSeizuresIndx, labels):
...
def calculatePerformanceAfterVariousSmoothing(predLabels, label, probabilityLabels,toleranceFP_bef, toleranceFP_aft, numLabelsPerHour, seizureStableLenToTestIndx, seizureStablePercToTest,  distanceBetweenSeizuresIndx, probThresh):
    ...
    performancesAll=torch.empty((numPerf*numTypes))
    ...
    #bayes smoothing
    (_,yPred_SmoothBayes)=smoothenLabels_Bayes(predLabels, probabilityLabels, seizureStableLenToTestIndx, probThresh, distanceBetweenSeizuresIndx, label)
