...
- outputs predictions and performance when adding one by one feature/channel (or certain perfcentage of feature/channels) that are sorted based on VotingParam.approach from above 
- plots:
...
class VotingParam:
    # choose approach for choosing next feature/channel added (most confident/the one with best performance or optimal taking into account also correlation)
    approach = 'OptimalOrder' #'Confidence', 'Performance', 'OptimalOrder'
    # choose in what steps are elements added -  either one by one feature/channel or percentage of feature/channels
...
#choose weather analysis per feature or channel |||
...
#
# # distSeparability_AllSubj = np.zeros((len(GeneralParams.patients), numElems + 1))
# featureOrders_allSubj=np.zeros((len(GeneralParams.patients), numElems))
...
#     filesAll = np.sort(glob.glob(folderOutPredictionsPerFeat + '/*Subj' + pat + '*' + HDtype + '_'+FeatChType+'_TrainPredictions.csv.gz'))
#     numFiles = len(filesAll)
#     print('-- Patient:', pat, 'NumSeizures:', numFiles)
#
#     # distSeparability_ThisSubj = np.zeros((numFiles, numElems + 1))
#     featureOrders_ThisSubj = np.zeros((numFiles, numElems ))
...
#
#         # load predictions
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_TrainPredictions.csv.gz'
#         data0 = readDataFromFile(fileInName)
#         predLabels_train = data0[:, 0:-1]
#         label_train = data0[:, -1]
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_TestPredictions.csv.gz'
#         data0 = readDataFromFile(fileInName)
#         predLabels_test = data0[:, 0:-1]
#         label_test = data0[:, -1]
#
#         # load distances from seiz and non seiz class
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_DistancesFromSeiz_Train.csv.gz'
#         distFromS_train = readDataFromFile(fileInName)
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_DistancesFromNonSeiz_Train.csv.gz'
#         distFromNS_train = readDataFromFile(fileInName)
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_DistancesFromSeiz_Test.csv.gz'
#         distFromS_test = readDataFromFile(fileInName)
#         fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_DistancesFromNonSeiz_Test.csv.gz'
...
#         if ( VotingParam.approach=='Confidence'):
#             fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_TrainConfidence.csv.gz'
...
#         elif ( VotingParam.approach=='Performance'):
#             fileInName = folderOutPredictionsPerFeat + '/' + fileName2 + '_'+FeatChType+'_TrainPerformance.csv.gz'
#             featureQualityMeasure = readDataFromFile(fileInName)
#             featureQualityMeasure=featureQualityMeasure[0:-1,7] #2 for F1E, 7 for F1DE
#
#         elif (VotingParam.approach=='OptimalOrder'):
#             if (HDParams.bindingFeatures == 'FeatAppend'):
#                 fileInName = folderOutFeatDiffMeas + '/FeatureOptimalOrder_' +HDtype+'_F1DE/' + fileName2 + '_TrainOptimalOrdering.csv.gz'  #[:-2]
#             else:
#                 fileInName = folderOutFeatDiffMeas + '/ChannelOptimalOrder_'+HDtype+'_F1DE/' + fileName2 + '_TrainOptimalOrdering.csv.gz'#[:-2]
#             featureQualityMeasure = readDataFromFile(fileInName)[1,:]
#             featureQualityMeasure=np.abs(featureQualityMeasure-numElems) #to reverse that bigger number means better
#
#         #feature order
#         sortIndx=np.argsort(-featureQualityMeasure)
#         orderedIndx=np.arange(1,numElems+1,1)
...
#
#         # saving
#         outputName = folderOutPredictionsVoting + '/' + fileName2 + '_Performance'+FeatChType+'_Train.csv'
#         saveDataToFile(performancesAll_Train, outputName, 'gzip')
#         outputName = folderOutPredictionsVoting + '/' + fileName2 + '_Performance'+FeatChType+'_Test.csv'
...
#     outputName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_Performance'+FeatChType+'_Train.csv'
...
#     outputName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_Performance'+FeatChType+'_Test.csv'
#     saveDataToFile(dataToSave, outputName, 'gzip')
#
#     outputName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_'+FeatChType+'Orders.csv'
...
featureOrders_allSubj=np.zeros((len(GeneralParams.patients), numElems))
#load performances as adding one by one feature/channel and also orders of features
for patIndx, pat in enumerate(GeneralParams.patients):
    filesAll = np.sort( glob.glob(folderOutPredictionsPerFeat + '/*Subj' + pat + '*' + HDtype + '_'+FeatChType+'_TrainPredictions.csv.gz'))
    numFiles = len(filesAll)
    print('-- Patient:', pat, 'NumSeizures:', numFiles)
    fileInName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_Performance'+FeatChType+'_Train.csv'
    performancesAll_Train_AllSubj[:, :, patIndx] = readDataFromFile(fileInName)[:, 0:36]
    fileInName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_Performance'+FeatChType+'_Test.csv'
    performancesAll_Test_AllSubj[:, :, patIndx] = readDataFromFile(fileInName)[:, 0:36]
    fileInName = folderOutPredictionsVoting + '/Subj' + pat + '_' + HDtype + '_'+FeatChType+'Orders.csv'
    featureOrders=readDataFromFile(fileInName)
    featureOrders_allSubj[patIndx,:]=np.mean(featureOrders,0)

# calcualte and save avrg of all subj
dataToSave = np.zeros((numThr + 1, 72))
dataToSave[:, 0:36] = np.nanmean(performancesAll_Train_AllSubj, 2)
dataToSave[:, 36:] = np.nanstd(performancesAll_Train_AllSubj, 2)
outputName = folderOutPredictionsVoting + '/AllSubj_' + HDtype + '_Performance'+FeatChType+'_Train.csv'
saveDataToFile(dataToSave, outputName, 'gzip')
dataToSave = np.zeros((numThr + 1, 72))
dataToSave[:, 0:36] = np.nanmean(performancesAll_Test_AllSubj, 2)
dataToSave[:, 36:] = np.nanstd(performancesAll_Test_AllSubj, 2)
outputName = folderOutPredictionsVoting + '/AllSubj_' + HDtype + '_Performance'+FeatChType+'_Test.csv'
saveDataToFile(dataToSave, outputName, 'gzip')
outputName = folderOutPredictionsVoting + '/AllSubj_' + HDtype + '_'+FeatChType+'Orders.csv'
...
for perfIndx, perf in enumerate(perfIndxs):
    ...
    ax1.errorbar(xValues, np.nanmean(performancesAll_Train_AllSubj[sindx:-1, 18+perf, :], 1),
                 yerr=np.nanstd(performancesAll_Train_AllSubj[sindx:-1, 18+perf, :], 1), color='darkorchid')
    ax1.legend(['Raw', 'Postprocessed'], fontsize=LettersSize)
    ...
    ax1.errorbar(xValues, np.nanmean(performancesAll_Test_AllSubj[sindx:-1, 18+perf, :], 1),
                 yerr=np.nanstd(performancesAll_Test_AllSubj[sindx:-1, 18+perf, :], 1), color='darkorchid', linestyle='--')
    ax1.set_xticks(xValues)
    ax1.set_ylim([0.2,1.1])
    if (VotingParam.selectionStyle == 'Perc'):
        ax1.set_xticklabels(thrNames[sindx:-1], fontsize=LettersSize * 0.8, rotation=45)
    else:
        ax1.set_xticklabels(thrNames[sindx:-1], fontsize=LettersSize * 0.8)
        if (FeatChType=='PerFeat'):
            ax1.set_xlabel('Number of features', fontsize=LettersSize)
        else:
            ax1.set_xlabel('Number of channels', fontsize=LettersSize)
    ax1.set_title(perfNames[perfIndx], fontsize=LettersSize)
    ax1.grid()
xValues = np.arange(1, numElems +1, 1)
ax1 = fig1.add_subplot(gs[0, 2])
ax1.boxplot(featureOrders_allSubj, medianprops=dict(color='darkorchid', linewidth=1),
                boxprops=dict(linewidth=1), capprops=dict(linewidth=1), whiskerprops=dict(linewidth=1), showfliers=False)
if (FeatChType=='PerFeat'):
    ax1.set_title('Order of feature per importance', fontsize=LettersSize)
else:
    ax1.set_title('Order of channel per importance', fontsize=LettersSize)
ax1.grid()
ax1.set_xticks(xValues)
ax1.set_xticklabels(elemNames, fontsize=LettersSize * 0.8, rotation=45)
# ax1.grid()
fig1.show()
...
for patIndx, pat in enumerate(GeneralParams.patients):
    filesAll = np.sort(glob.glob( folderOutPredictionsVoting + '/*Subj' + pat + '_cv*' + HDtype + '_Performance'+FeatChType+'_Train.csv.gz'))
    ...
    for fIndx in range(len(filesAll)):
        ...
        if (FeatChType == 'PerFeat'):
            ...
            fileName2 = os.path.splitext(fileName1)[0][0:-36]
...
# Plotting
fig1 = plt.figure(figsize=(20,3), constrained_layout=False) #5,10
gs = GridSpec(1, 3, figure=fig1)
fig1.subplots_adjust(wspace=0.15, hspace=0.15)
...
ax1.set_xlabel('Subjects', fontsize=LettersSize)
ax1.set_xticks(xValues)
ax1.set_xticklabels(xValues.astype(int), fontsize=LettersSize * 0.8)
ax1.set_title('Optimal threshold, mean='+'{:.2f}'.format(np.mean(optThr_allSubj[0, :])), fontsize=LettersSize)
ax1.grid()
...
ax1.errorbar(xValues, perfImprovTest_allSubj[:, 2, 0], yerr=perfImprovTest_allSubj[:, 2, 1], color='darkorchid', linestyle='None', marker='D', markersize=5.0)
ax1.legend(['Train', 'Test'], fontsize=LettersSize)
ax1.plot(xValues, np.ones(len(xValues)) * np.mean(perfImprovTrain_allSubj[:, 2, 0]), 'k--', linewidth=2)
ax1.plot(xValues, np.ones(len(xValues)) * np.mean(perfImprovTest_allSubj[:, 2, 0]), color='darkorchid', linestyle='--', linewidth=2)
ax1.set_xlabel('Subjects', fontsize=LettersSize)
ax1.set_xticks(xValues)
ax1.set_xticklabels(xValues.astype(int), fontsize=LettersSize * 0.8)
ax1.set_title('F1E performance improvement, mean='+'{:.2f}'.format(100*np.mean(perfImprovTest_allSubj[:, 2, 0]) )+'%', fontsize=LettersSize)
ax1.grid()
ax1 = fig1.add_subplot(gs[0, 2])
...
ax1.errorbar(xValues, perfImprovTest_allSubj[:, 2, 0], yerr=perfImprovTest_allSubj[:, 7, 1], color='darkorchid', linestyle='None', marker='D',markersize=5.0)
ax1.legend(['Train', 'Test'], fontsize=LettersSize)
ax1.plot(xValues, np.ones(len(xValues)) * np.mean(perfImprovTrain_allSubj[:, 7, 0]), 'k--', linewidth=2)
ax1.plot(xValues, np.ones(len(xValues)) * np.mean(perfImprovTest_allSubj[:, 7, 0]), color='darkorchid', linestyle='--', linewidth=2)
ax1.set_xlabel('Subjects', fontsize=LettersSize)
ax1.set_xticks(xValues)
ax1.set_xticklabels(xValues.astype(int), fontsize=LettersSize * 0.8)
ax1.set_title('F1DE performance improvement, mean='+'{:.2f}'.format(100*np.mean(perfImprovTest_allSubj[:, 7, 0])) +'%', fontsize=LettersSize)
ax1.grid()
